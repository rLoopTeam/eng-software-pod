/**
 * @file		rm4_system__mpu.h
 * @brief		Memory protection unit main header.
...
*     - MPU_PRIV_NA_USER_NA_EXEC no access in privileged u32Mode, no access in user u32Mode and execute
*     - MPU_PRIV_RW_USER_NA_EXEC read/write in privileged u32Mode, no access in user u32Mode and execute
*     - MPU_PRIV_RW_USER_RO_EXEC read/write in privileged u32Mode, read only in user u32Mode and execute
*     - MPU_PRIV_RW_USER_RW_EXEC read/write in privileged u32Mode, read/write in user u32Mode and execute
*     - MPU_PRIV_RO_USER_NA_EXEC read only in privileged u32Mode, no access in user u32Mode and execute
*     - MPU_PRIV_RO_USER_RO_EXEC read only in privileged u32Mode, read only in user u32Mode and execute
*     - MPU_PRIV_NA_USER_NA_NOEXEC no access in privileged u32Mode, no access in user u32Mode and no execution
*     - MPU_PRIV_RW_USER_NA_NOEXEC read/write in privileged u32Mode, no access in user u32Mode and no execution
*     - MPU_PRIV_RW_USER_RO_NOEXEC read/write in privileged u32Mode, read only in user u32Mode and no execution
*     - MPU_PRIV_RW_USER_RW_NOEXEC read/write in privileged u32Mode, read/write in user u32Mode and no execution
*     - MPU_PRIV_RO_USER_NA_NOEXEC read only in privileged u32Mode, no access in user u32Mode and no execution
*     - MPU_PRIV_RO_USER_RO_NOEXEC read only in privileged u32Mode, read only in user u32Mode and no execution
*
*/
enum mpuRegionAccessPermission
{
    MPU_PRIV_NA_USER_NA_EXEC   = 0x0000U, /**< Alias no access in privileged u32Mode, no access in user u32Mode and execute */
    MPU_PRIV_RW_USER_NA_EXEC   = 0x0100U, /**< Alias no read/write in privileged u32Mode, no access in user u32Mode and execute */
    MPU_PRIV_RW_USER_RO_EXEC   = 0x0200U, /**< Alias no read/write in privileged u32Mode, read only in user u32Mode and execute */
    MPU_PRIV_RW_USER_RW_EXEC   = 0x0300U, /**< Alias no read/write in privileged u32Mode, read/write in user u32Mode and execute */
    MPU_PRIV_RO_USER_NA_EXEC   = 0x0500U, /**< Alias no read only in privileged u32Mode, no access in user u32Mode and execute */
    MPU_PRIV_RO_USER_RO_EXEC   = 0x0600U, /**< Alias no read only in privileged u32Mode, read only in user u32Mode and execute */
    MPU_PRIV_NA_USER_NA_NOEXEC = 0x1000U, /**< Alias no access in privileged u32Mode, no access in user u32Mode and no execution */
    MPU_PRIV_RW_USER_NA_NOEXEC = 0x1100U, /**< Alias no read/write in privileged u32Mode, no access in user u32Mode and no execution */
    MPU_PRIV_RW_USER_RO_NOEXEC = 0x1200U, /**< Alias no read/write in privileged u32Mode, read only in user u32Mode and no execution */
    MPU_PRIV_RW_USER_RW_NOEXEC = 0x1300U, /**< Alias no read/write in privileged u32Mode, read/write in user u32Mode and no execution */
    MPU_PRIV_RO_USER_NA_NOEXEC = 0x1500U, /**< Alias no read only in privileged u32Mode, no access in user u32Mode and no execution */
    MPU_PRIV_RO_USER_RO_NOEXEC = 0x1600U  /**< Alias no read only in privileged u32Mode, read only in user u32Mode and no execution */
};
...
/** @fn Luint32 _mpuGetNumberOfRegions_(void)
...
*/
Luint32 _mpuGetNumberOfRegions_(void);

/** @fn Luint32 _mpuAreRegionsSeparate_(void)
...
*/
Luint32 _mpuAreRegionsSeparate_(void);

/** @fn void _mpuSetRegion_(Luint32 region)
*   @brief Set mpu region number
*
*   This function selects one of the implemented mpu regions.
*/
void _mpuSetRegion_(Luint32 region);

/** @fn Luint32 _mpuGetRegion_(void)
...
*/
Luint32 _mpuGetRegion_(void);

/** @fn void _mpuSetRegionBaseAddress_(Luint32 address)
...
*/
void _mpuSetRegionBaseAddress_(Luint32 address);

/** @fn Luint32 _mpuGetRegionBaseAddress_(void)
...
*/
Luint32 _mpuGetRegionBaseAddress_(void);

/** @fn void _mpuSetRegionTypeAndPermission_(Luint32 type, Luint32 permission)
*   @brief Set type of currently selected mpu region
*
*   This function sets the type of currently selected mpu region.
*/
void _mpuSetRegionTypeAndPermission_(Luint32 type, Luint32 permission);

/** @fn Luint32 _mpuGetRegionType_(void)
...
*/
Luint32 _mpuGetRegionType_(void);

/** @fn Luint32 _mpuGetRegionPermission_(void)
...
*/
Luint32 _mpuGetRegionPermission_(void);

/** @fn void _mpuSetRegionSizeRegister_(Luint32 value)
*   @brief Set mpu region size register value
*
*   This function sets mpu region size register value.
*/
void _mpuSetRegionSizeRegister_(Luint32 value);
