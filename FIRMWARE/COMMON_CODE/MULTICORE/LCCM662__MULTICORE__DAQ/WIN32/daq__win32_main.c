...
DLL_DECLARATION void vDAQ_WIN32_MAIN__Init(void)
{
	Luint8 u8Test;
	...
	//config our packet types
		...
		vDAQ__Config_UserPacketType(0U, 0x5000);
	#endif
	...
	vETHERNET__Init(&u8MACAddx[0], &u8IPAddx[0]);

}

//process the loop
DLL_DECLARATION void vDAQ_WIN32_MAIN__Process(void)
{
	Lfloat32 f32Temp1;
	Lfloat32 f32Temp2;
	Luint8 u8Test;
...
//U8
DLL_DECLARATION void vDAQ_WIN32_MAIN__1HZ_ISR(Luint8 u8Value)
{
	//cpu load
	vDAQ_APPEND__U8(0, u8Value);
}


void vDAQ_WIN32_MAIN__RxUDPSafe(Luint8 *pu8Payload, Luint16 u16PayloadLength, SAFE_UDP__PACKET_T ePacketType, Luint16 u16DestPort, SAFE_UDP__FAULTS_T eFault)
{
	...
	switch((Luint16)ePacketType)
	{
		...
		case SAFE_UDP__LCCM662__DAQ_STREAMING_CONTROL:
			...
			u32Temp = u32NUMERICAL_CONVERT__Array(pu8Payload + 0);
