/**
 * @file		BQ76__CRC.C
 * @brief		CRC Functions
 * @author		Lachlan Grogan
 * @copyright	This file contains proprietary and confidential information of
 *				SIL3 Pty. Ltd. (ACN 123 529 064). This code may be distributed
 *				under a license from SIL3 Pty. Ltd., and may be used, copied
 *				and/or disclosed only pursuant to the terms of that license agreement.
 *				This copyright notice must be retained as part of this file at all times.
 * @copyright	This file is copyright SIL3 Pty. Ltd. 2003-2016, All Rights Reserved.
 * @st_fileID	LCCM715R0.FILE.009
 */

/**
 * @addtogroup MULTICORE
 * @{
*/
/**
 * @addtogroup BQ76
 * @ingroup MULTICORE
 * @{
*/
/**
 * @addtogroup BQ76__CRC
 * @ingroup BQ76
 * @{
*/
#include <MULTICORE/LCCM715__MULTICORE__BQ76PL536A/bq76.h>
#if C_LOCALDEF__LCCM715__ENABLE_THIS_MODULE == 1U

//CRC-8, poly = x^8 + x^2 + x^1 + x^0, init = 0
const Luint8 u8BQ76_CRCTable[256] = 
{
	
};
  
  
extern TS_BQ76__MAIN sBQ76;


/***************************************************************************//**
 * @brief
 * Compute the CRC
 * 
 * @note
 * CRC-8, poly = x^8 + x^2 + x^1 + x^0, init = 0
 *
 * @param[in]		u8Length			Total length of data
 * @param[in]		*pu8Buffer			Pointer to the data buffer
 * @return			Return the computed CRC
 * @st_funcMD5		0C203605FA32BFB89DC994CA0A081087
 * @st_funcID		LCCM715R0.FILE.009.FUNC.001
 */
Luint8 u8BQ76_CRC__Compute(Luint8 *pu8Buffer, Luint8 u8Length)
{
	Luint8 u8CRC;
	Luint8 u8Counter;
	Luint8 u8Temp;

	//initial conditions
	u8CRC = 0;

	for(u8Counter = 0; u8Counter < u8Length; u8Counter++)
	{
		u8Temp = u8CRC ^ pu8Buffer[u8Counter];
		
		u8CRC = u8BQ76_CRCTable[u8Temp];
	}
	
	return u8CRC;
}



#endif //#if C_LOCALDEF__LCCM715__ENABLE_THIS_MODULE == 1U
//safetys
#ifndef C_LOCALDEF__LCCM715__ENABLE_THIS_MODULE
	#error
#endif
/** @} */
/** @} */
/** @} */

