...
void vPWRNODE_BATTTEMP_MEM__Init(void)
{
	...
	u8Test = u8SIL3_EEPARAM_CRC__Is_CRC_OK(	C_POWERCORE__EEPARAM_INDEX__BATTTEMP__NUM_SENSORS,
	...
	{
		//CRC was good
		sPWRNODE.sTemp.u16NumSensors = u16SIL3_EEPARAM__Read(C_POWERCORE__EEPARAM_INDEX__BATTTEMP__NUM_SENSORS);
		sPWRNODE.sTemp.u16PackMemCRC = u16SIL3_EEPARAM__Read(C_POWERCORE__EEPARAM_INDEX__BATTTEMP__PACK_CRC);
	}
	else
	{

		//default the data
		vSIL3_EEPARAM__WriteU16(	C_POWERCORE__EEPARAM_INDEX__BATTTEMP__NUM_SENSORS,
							((C_PWRCORE__NUM_TEMP_SNSR_PER_6P * C_PWRCORE__NUM_6P_MODULES) + 3U),
							DELAY_T__DELAYED_WRITE);
		//reload it
		sPWRNODE.sTemp.u16NumSensors = u16SIL3_EEPARAM__Read(C_POWERCORE__EEPARAM_INDEX__BATTTEMP__NUM_SENSORS);
		...
		//update our CRC
		vSIL3_EEPARAM_CRC__Calculate_And_Store_CRC(	C_POWERCORE__EEPARAM_INDEX__BATTTEMP__NUM_SENSORS,
												C_POWERCORE__EEPARAM_INDEX__BATTTEMP__PACK_CRC,
												C_POWERCORE__EEPARAM_INDEX__BATTTEMP__CRC);
...
Lint16 s16PWRNODE_BATTEMP_MEM__Load(void)
{
	...
	//do the CRC on it
	u16CRC = 0U;
	u16CRC = u16SIL3_SWCRC__CRC(pu8Array, u16MemorySize);
