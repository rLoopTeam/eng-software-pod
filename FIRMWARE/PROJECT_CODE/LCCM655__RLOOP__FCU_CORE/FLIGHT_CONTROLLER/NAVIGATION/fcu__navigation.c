...
void vFCU_FLIGHTCTL_NAV__Init(void)
{
	...
	// initialize each CS
	sFCU.sNavigation.sCS[0].u32NoseToSensorDist = C_FCU__NAV_DISTANCE_BETWEEN_CONTRAST_SENSOR_1_AND_NOSE;
	sFCU.sNavigation.sCS[1].u32NoseToSensorDist = C_FCU__NAV_DISTANCE_BETWEEN_CONTRAST_SENSOR_2_AND_NOSE;
	sFCU.sNavigation.sCS[2].u32NoseToSensorDist = C_FCU__NAV_DISTANCE_BETWEEN_CONTRAST_SENSOR_3_AND_NOSE;
	for (u8CSIndex = 0; u8CSIndex < C_FCU__NAV_NUM_CONTRAST_SENSORS; u8CSIndex++)
	{
		...
		sFCU.sNavigation.sCS[0].u32Xpos = C_FCU__NAV_STARTUP_REAR_X_POSITION + C_FCU__NAV_POD_LENGTH - sFCU.sNavigation.sCS[0].u32NoseToSensorDist;
...
Lint8 searchStripsInRange(Luint8 u8CS)
{
	Luint8 u8Index;
	...
	for (u8Index = u32CurStripeCount; u8Index < C_FCU__LASER_CONTRAST__MAX_STRIPES; u8Index ++)
	{
	...
	return -1;	// returning -1 instead of 0 because 0 is actually a valid stripe number
}
...
void vFCU_FLIGHTCTL_NAV__ContrastSensor(void)
{
	...
	for (u8CSIndex = 0; u8CSIndex < C_FCU__NAV_NUM_CONTRAST_SENSORS; u8CSIndex++)
	{
		...
		if (u8FCU_LASERCONT_TL__Get_NewRisingAvail(Get_LC_Enum(u8CSIndex)))
		{
			...
			if (sFCU.sNavigation.sCS[u8CSIndex].u8FirstRisingFlag == 0)
			{
				...
				sFCU.sNavigation.sCS[u8CSIndex].u8FirstRisingFlag = 1;
			}
		}
		else
		{
			// not detected
			if (sFCU.sNavigation.sCS[u8CSIndex].u8FirstRisingFlag == 1)
			{
				// just finished detecting previous stripe
				// process here, rigth after detecting
				if (s8ExpectedStripeNum)
				{
					// expected and detected
					// disregarding " If several stripes are in the area, the one next to the general stripe counter shall be used."
					// because difficult to  figure out many pulses
					sFCU.sNavigation.sCS[u8CSIndex].u32StripeCount = s8ExpectedStripeNum;
					sFCU.sNavigation.sCS[u8CSIndex].u32Xpos = sFCU.sContrast.sTrackDatabase[u8CS].u32DistanceElapsed_mm[s8ExpectedStripeNum];
					sFCU.sNavigation.sCS[u8CSIndex].u32XPosUncert = C_FCU__NAV_X_POS_UNCERTAINTY_OFFSET;
					sFCU.sNavigation.f32LongitudinalSpeed = C_FCU__NAV_STRIPE_LENGTH/sFCU.sNavigation.u3210MSDetectStripeTimer;
					sFCU.sNavigation.f32LongitudinalSpeedUncertainty = C_FCU__NAV_SPEED_UNCERTAINTY_OFFSET;
				}
				else
				{
					// detected previously but was not expected
					// use last values
				...
				sFCU.sNavigation.sCS[u8CSIndex].u8FirstRisingFlag = 0;
				sFCU.sNavigation.u3210MSDetectStripeTimer = 0;
			}
			if (s8ExpectedStripeNum)
			{
			...
			{
				// in between stripes
				// TODO:  how do we compute pos, speed and acceleration without accelerometers here?
			}
	...
	//see if in area for laser range finder
	//reset timer so can do transition blending
	if (u8InLaserRangeFinderArea())
...
void vFCU_FLIGHTCTL_NAV__Process(void)
{
	...
	if (u8InLaserRangeFinderArea() && (sFCU.sLaserDist.eLaserState != LASERDIST_STATE__RESET))	//TODO: check lrf working?
	{
		...
		sFCU.sNavigation.f32LongitudinalSpeed = sFCU.sLaserDist.s32Velocity_mms;
		sFCU.sNavigation.f32LongitudinalAcceleration = sFCU.sLaserDist.s32Accel_mmss;
	}
