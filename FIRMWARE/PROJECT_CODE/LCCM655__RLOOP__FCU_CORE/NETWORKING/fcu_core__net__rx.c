		...
		switch((E_NET__PACKET_T)ePacketType)
		{
			...
			case NET_PKT__FCU_GEN__POD_COMMAND:
				...
				if(u32Block[0] == 0x4321FEDCU)  // Unlock key
				{
					...
					{
						vFCU_FCTL__NetCommand_Unlock((TE_POD_COMMAND_T)u32Block[1]);
					}
					else
					{
						// log an error?
				...
				else if(u32Block[0] == 0xDCBA9876U)  // Execute key
				{
					...
					if (u32Block[1] >= 0 && u32Block[1] < POD_COMMAND__NUM_COMMANDS)  // Check bounds
					{
						if ( vFCU_FCTL__NetCommand_IsEnabled((TE_POD_COMMAND_T)u32Block[1]) == 1 )
						{
							// @todo: change this to take a strPodCmd
							vFCU_FCTL__PushCommand((TE_POD_COMMAND_T)u32Block[1]);
						}
					}
					else
					{
